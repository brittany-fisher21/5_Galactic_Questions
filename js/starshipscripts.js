...
function getStarshipList () {
    ...
    .then(data => {
        createStarshipList(data)
    })
    .catch(error => {
        console.error("ERROR", error);
    });
    function createStarshipList (data) {
        const starship = data.results;
        starship.forEach(function (starship) {
    ...
    if (data.next !== null) {
        ...
        let filteredArray = starshipArray.filter(filterArray)
        chooseStarship(filteredArray)
    }
